...
*/
  ...
  //----------------------------------------------------------------------------
  /*function CreateListPost0(category_post) {
    console.log("start createlistpost");
    console.log(category_post, category_post.id);
    let user_id = category_post.post_user_name[1];
    const element_post = document.createElement('div');
    element_post.classList.add("childposts");
    element_post.innerHTML = `<a class="stranger-link" href="/index/${user_id}">${category_post.post_user_name[0]}</a> ->: <br>
                              ${category_post.post_text} <br>
                              <span id="chainline">${category_post.post_date}</span> <br>
                              Like: ${category_post.post_like} `;
    element_post.addEventListener('click', function() {
    console.log('this element click!');
    });
    console.log(element_post);

    document.querySelector('.posts').append(element_post);
...
function SeeComment(post_id) {
  ...
  if (see_comment.checked) {
    ...
    switcher = "False";
    console.log(switcher)
    //comment_div.style.animationName = "blockcomments";
   // comment_div.style.animationDirection = 'revers';
   // comment_div.style.animationPlayState = 'running';
...
function ShowMeFollowing(subscribe_unsubscribe) {
  ...
  revers_subscribe.forEach(function(follow){
   ...
   /* const element_post = document.createElement('div');
    element_post.classList.add("childposts");
    element_post.innerHTML = `<a class="stranger-link" href="##">${follow.following_user_id[0]}</a> ->: <br>
                               ${follow.subscribed_date}<br>`;
    element_post.addEventListener('click', function() {
    console.log('this element click!');
    });
    console.log(element_post);*/
